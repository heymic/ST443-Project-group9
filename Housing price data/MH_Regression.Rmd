...
X_test <- model.matrix(~.-1, data=test_kaggle)
```
...
```{r, eval = FALSE}
prediction_RF_p3_log <- predict(RF_p3.SalePrice, newdata = test_kaggle)
...
```{r,  eval = FALSE}
prediction_RF_tuned_log <- predict(RF.SalePrice_tuned, newdata = test_kaggle)
...
```{r, eval = FALSE}
prediction_RF_tune_log <- predict(bestmtry, newdata = test_kaggle)
...
```{r}
prediction_BOOST_log <- predict(boost.train, test_kaggle)
